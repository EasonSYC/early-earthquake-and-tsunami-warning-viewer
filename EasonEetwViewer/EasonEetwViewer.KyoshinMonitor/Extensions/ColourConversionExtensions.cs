...
public static class ColourConversionExtensions
{
    ...
    public static double IntensityToHeight(this double intensity)
        => (intensity + 3) / 10;
    /// <summary>
    ...
    /// <returns>The normalised height.</returns>
    public static double PgaToHeight(this double pga)
        => (Math.Log(pga, 10) + 2) / 5;
    /// <summary>
    ...
    /// <returns>The normalised height.</returns>
    public static double PgvToHeight(this double pgv)
        => (Math.Log(pgv, 10) + 3) / 5;
    /// <summary>
    ...
    /// <returns>The normalised height.</returns>
    public static double PgdToHeight(this double pgd)
        => (Math.Log(pgd, 10) + 4) / 5;
    /// <summary>
    ...
    public static double HeightToIntensity(this double height)
        => (10 * height) - 3;
    /// <summary>
    ...
    public static double HeightToPga(this double height)
        => Math.Pow(10, (5 * height) - 2);
    /// <summary>
    /// Converts normalised height to PGV. 
    /// </summary>
    /// <param name="height">The normalised height.</param>
    /// <returns>The Peak Ground Velocity.</returns>
    public static double HeightToPgv(this double height)
        => Math.Pow(10, (5 * height) - 3);
    /// <summary>
    /// Converts normalised height to PGD.
    /// </summary>
    /// <param name="height">The normalised height.</param>
    /// <returns>The Peak Ground Displacement.</returns>
    public static double HeightToPgd(this double height)
        => Math.Pow(10, (5 * height) - 4);
    /// <summary>
    /// Converts normalised height to hue.
    /// </summary>
    /// <param name="height">The normalised height.</param>
    /// <returns>The hue component of the colour.</returns>
    public static double HeightToHue(this double height)
        => height switch
        {
            <= 0.1 => (-150 * height) + 237,
            >= 0.1 and <= 0.6 => (222 * (height - 0.3) * (height - 0.4) * (height - 0.6) / ((0.1 - 0.3) * (0.1 - 0.4) * (0.1 - 0.6)))
    ...
    public static double HeightToSaturation(this double height)
        => height switch
        {
            <= 0.2 => 1,
            >= 0.2 and <= 0.29 => (-2.611 * height) + 1.522,
            >= 0.29 and <= 0.4 => (1.682 * height) + 0.277,
            >= 0.4 and <= 0.5 => (0.5 * height) + 0.75,
            _ => 1
        };
    /// <summary>
    ...
    public static double HeightToValue(this double height)
        => height switch
        {
            <= 0.1 => (1.8 * height) + 0.8,
            >= 0.1 and <= 0.172 => (-4.444 * height) + 1.424,
            >= 0.172 and <= 0.2 => (5.714 * height) - 0.323,
            >= 0.2 and <= 0.3 => (1.6 * height) + 0.5,
            >= 0.3 and <= 0.4 => (0.2 * height) + 0.92,
            >= 0.4 and <= 0.8 => 1,
            >= 0.8 and <= 0.9 => (-0.3 * height) + 1.24,
            _ => (-2.9 * height) + 3.58
        };
    /// <summary>
    ...
    public static SKColor HeightToColour(this double height)
        => SKColor.FromHsv((float)height.HeightToHue(), (float)height.HeightToSaturation() * 100, (float)height.HeightToValue() * 100);
    /// <summary>
